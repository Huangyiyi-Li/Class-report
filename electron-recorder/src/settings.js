...
import { atomicWriteJson, validateBootstrapDataRoot } from "./worker-bootstrap.js";
...
const RUNTIME_KEYS = new Set(["autoRecordEnabled", "inputDevice", "dataRoot"]);
...
function validateText(value, name, maxLength, { allowEmpty = false } = {}) {
  ...
  if (!allowEmpty && !normalized) throw new TypeError(`${name} must not be empty`);
  if (normalized.length > maxLength) throw new TypeError(`${name} exceeds ${maxLength} characters`);
  if (/\0|[\r\n]/u.test(normalized)) throw new TypeError(`${name} contains invalid characters`);
  return normalized;
}

export function validateAutoLaunchValue(value) {
  if (typeof value !== "boolean") throw new TypeError("autoLaunch must be boolean");
  return value;
}
...
export function validateSettingsPatch(patch, options = {}) {
  ...
  if (unknown.length) throw new TypeError(`settings patch contains forbidden field: ${unknown[0]}`);
  const validated = {};
  if (Object.hasOwn(patch, "autoRecordEnabled")) {
    if (typeof patch.autoRecordEnabled !== "boolean") throw new TypeError("autoRecordEnabled must be boolean");
    validated.autoRecordEnabled = patch.autoRecordEnabled;
  }
  if (Object.hasOwn(patch, "inputDevice")) validated.inputDevice = validateText(patch.inputDevice, "inputDevice", 256);
...
function validatePersisted(payload) {
  ...
  if (Object.keys(payload).some((key) => !PERSISTED_KEYS.has(key))) throw new TypeError("persisted settings contain unknown fields");
  return {
    autoLaunch: validateAutoLaunchValue(payload.autoLaunch),
    autoRecordEnabled: validateSettingsPatch({ autoRecordEnabled: payload.autoRecordEnabled }).autoRecordEnabled,
    inputDevice: validateSettingsPatch({ inputDevice: payload.inputDevice }).inputDevice,
  };
}
...
export function loadWorkerCoreSettings(configPath) {
  try {
    ...
    if (typeof payload.auto_record_enabled !== "boolean" || typeof payload.input_device !== "string") return {};
    const inputDevice = payload.input_device.trim();
    if (inputDevice.length > 256 || /\0|[\r\n]/u.test(inputDevice)) return {};
    return { autoRecordEnabled: payload.auto_record_enabled, inputDevice: inputDevice || "default" };
  } catch {
    return {};
  }
}
...
export function applyAutoLaunch({ desired, app, platform = process.platform }) {
  ...
  if (platform !== "win32") {
    return { desired, actual: null, status: "unverified", error: "仅支持在 Windows 上验证开机自启状态" };
  }
  try {
    app.setLoginItemSettings({ openAtLogin: desired, openAsHidden: true, path: process.execPath });
  } catch (error) {
    return { desired, actual: null, status: "failed", error: `Windows 开机自启设置失败: ${error.message}` };
  }
  try {
    const actual = app.getLoginItemSettings().openAtLogin === true;
    return { desired, actual, status: actual === desired ? "verified" : "unverified", error: null };
  } catch (error) {
    return { desired, actual: null, status: "failed", error: `Windows 开机自启状态读取失败: ${error.message}` };
  }
}
