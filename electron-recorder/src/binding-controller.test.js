...
function createFixture(overrides = {}) {
  ...
  const service = {
    ...
    cancelSession: async (id) => calls.push(["cancelSession", id]),
    getSession: async (id) => (
  ...
  const controller = new BindingController({
    ...
    resetAuthentication: async () => calls.push(["resetAuthentication"]),
...
test("switching identity discards the binding session before clearing Passport state", async () => {
  const { controller, calls } = createFixture();

  const nextSession = await controller.restartSession("session-1");

  assert.equal(nextSession.status, "authenticated");
