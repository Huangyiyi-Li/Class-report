...
test("first settings save writes full config outside userData and a secret-free locator", () => {
  ...
  const result = bootstrapWorkerConfig({
    userDataDir: userData,
    patch: { dataRoot: root, autoRecordEnabled: true, inputDevice: "mic-1" },
    validationOptions: filesystemValidation,
  });
  ...
  assert.deepEqual(JSON.parse(locatorText), {
...
test("restart resolves the non-system config through locator", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "recorder-data-"));
  const userData = fs.mkdtempSync(path.join(os.tmpdir(), "recorder-user-"));
  const created = bootstrapWorkerConfig({
    userDataDir: userData,
    patch: { dataRoot: root },
    validationOptions: filesystemValidation,
  });
...
test("Windows bootstrap rejects missing and system-drive roots", () => {
  assert.throws(() =>
    validateBootstrapDataRoot("", { platform: "win32", systemDrive: "C:" })
  );
  assert.throws(() =>
    validateBootstrapDataRoot("C:\\Recorder", {
      platform: "win32",
      systemDrive: "C:",
    })
  );
  assert.throws(() =>
    validateBootstrapDataRoot("\\\\server\\share\\Recorder", {
      platform: "win32",
      systemDrive: "C:",
    })
  );
  assert.doesNotThrow(() =>
    validateBootstrapDataRoot("D:\\Recorder", {
      platform: "win32",
      systemDrive: "C:",
    })
  );
...
for (const state of ["idle", "recording"]) {
  test(`data root migration is rejected while ${state}`, () => {
    ...
    const userData = fs.mkdtempSync(path.join(os.tmpdir(), "recorder-user-"));
    const original = bootstrapWorkerConfig({
      userDataDir: userData,
      patch: { dataRoot: firstRoot },
      validationOptions: filesystemValidation,
    });
    assert.throws(
      () =>
        bootstrapWorkerConfig({
          userDataDir: userData,
          patch: { dataRoot: nextRoot },
          validationOptions: filesystemValidation,
        }),
      /首次部署后不可修改/
    );
    assert.deepEqual(
      loadWorkerLocator(userData, filesystemValidation),
      original
    );
    assert.equal(
      fs.existsSync(
        path.join(nextRoot, ".classroom-recorder", "worker-config.json")
      ),
      false
    );
  });
...
test("locator rejects relative, mismatched and escaped config paths before use", () => {
  ...
  fs.writeFileSync(
    locatorPath,
    JSON.stringify({ dataRoot: root, configPath: "relative.json" })
  );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      dataRoot: root,
      configPath: path.join(root, ".classroom-recorder", "worker-config.json"),
    })
  );
  fs.mkdirSync(path.join(root, ".classroom-recorder"), { recursive: true });
  fs.writeFileSync(
    path.join(root, ".classroom-recorder", "worker-config.json"),
    JSON.stringify({ data_root: `${root}-other` })
  );
  assert.equal(loadWorkerLocator(userData), null);
});
...
test("Windows locator rejects system-drive and UNC roots before config access", () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      dataRoot: "C:\\Recorder",
      configPath: "C:\\Recorder\\.classroom-recorder\\worker-config.json",
    })
  );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      configPath: "\\\\server\\share\\.classroom-recorder\\worker-config.json",
    })
  );
  assert.equal(loadWorkerLocator(userData, options), null);
});
