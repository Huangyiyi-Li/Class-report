...
contextBridge.exposeInMainWorld("recorderShell", {
  ...
  createReplacementBindingSession: () => ipcRenderer.invoke("binding:create-replacement-session"),
  getBindingSession: (sessionId) => ipcRenderer.invoke("binding:get-session", sessionId),
  listBindingGrades: (sessionId) => ipcRenderer.invoke("binding:list-grades", sessionId),
  listBindingClasses: (sessionId, query) => ipcRenderer.invoke("binding:list-classes", sessionId, query),
