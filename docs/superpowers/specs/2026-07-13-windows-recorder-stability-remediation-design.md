...
本机控制通道只服务于同一台设备上的 Electron 与录音 worker。首版必须保证单实例、认证、断线不停录、自动重连和设置成功确认，但不实现分布式事务级的命令结果协调。

如果 worker 已执行设置写入、但确认消息恰好在返回前丢失，客户端提示“保存结果未确认，请重新打开设置核对”。该低概率窗口作为已知风险记录，不增加持久命令结果日志、两阶段提交或服务端 deadline 取消机制。
