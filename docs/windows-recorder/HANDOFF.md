...
- 更新日期：2026-09-03
- 当前状态：`v0.2.0-codex.28` 自动恢复 `.27` 遗留待解绑状态，并在远端解绑失败时恢复原绑定，等待 Windows 真机复测
...
- 当前源码版本：`0.2.0-codex.28`
- 最新候选标签：`v0.2.0-codex.28`
- 最新状态：正式绑定接口已可用；本候选启动时自动撤销 `.27` 未完成的本地待解绑标记，使原绑定重新具备录音能力。新发起的解绑若在服务端失败或网络中断，客户端会恢复原绑定运行态；只有服务端确认解绑成功后才清除本地绑定。必须在 Windows 真机验证 `.27` 现场配置升级后直接录音、解绑失败回滚、解绑成功及重新绑定。
