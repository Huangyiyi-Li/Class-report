...
### 用户确认的正式版待办（2026-09-17）

以下事项已完成源码优化（2026-09-20），尚待正式候选安装包在 Windows 上验收；勾选表示真机验收完成，而非仅代码完成：

- [ ] **录音说明**：默认收起，只能通过顶部入口主动打开；检查首次启动、手动收起、关闭重开及状态刷新均不自动展开。
- [ ] **高级设置**：普通模式隐藏接口和技术面板；技术人员启动进程前设置 `RECORDER_MAINTENANCE_MODE=1` 可显示。主进程同步阻止普通模式修改接口地址，普通录音设置保存不受影响。这是防误操作入口，不是操作系统权限隔离。
- [ ] **诊断日志**：worker 启动后每 60 秒保存快照至录音数据目录的 `logs/diagnostics/worker-YYYY-MM-DD.jsonl`，关闭主窗口后仍由 worker 保存；UTC 日期保留近 7 天，每天最多两个 2 MiB 文件，达到容量后轮换。自动脱敏口令、Token、Cookie、访问密钥等字段和错误文本中的凭证。只清理自身命名的过期日志，写入失败不停止录音，无系统盘回退。普通设置页可打开日志文件夹；手动导出仍在维护模式。自动日志记录 worker 录音/上传状态，不包含仅存在于 Electron 内存的 Passport 窗口事件。
- [ ] **最近录音**：默认展示保存、上传结果；不再要求用户核对没有查询入口的“有效时段”。完整性未确认的补充信息收在“录音详情”，已确认的保存未完成和文件缺失仍直接展示。
- [ ] **上传凭证失败**：尚未证明解决，需取得当前失败时的诊断日志并核对服务端响应；不将此前已经处理的 CORS 配置问题再次认定为根因。
